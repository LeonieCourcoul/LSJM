...
#' They are defined as the predicted probability of having event \eqn{k} between time \eqn{s} and
#' \eqn{s+t}
...
#' denoted by \eqn{Y_i(s)}, and the set of estimated parameters. The prediction is defined for
#' subject \eqn{i} by:
#' #' \deqn{
#' \pi_i^{0k}(s,t;\widehat{\theta}) =
#' P(s<T_i<s+t, \delta_i = k \mid T_i > s,
#' \mathcal{Y}_i(s), \widehat{\theta})
#' =
#' \frac{
#'   \int
#'   \left[
#'     \int_s^{s+t}
#'     \exp\left(
#'       -\sum_{c=1}^K \Lambda_{i}^{0c}(u|r_i,\widehat{\theta})
#'     \right)
#'     \lambda_{i}^{0k}(u|r_i,\widehat{\theta})
#'     du
#'   \right]
#'   f(\mathcal{Y}_i(s)|r_i,\widehat{\theta})
#'   f(r_i|\widehat{\theta})
#'   dr_i
#' }{
#'   \int
#'   \exp\left(
#'     -\sum_{c=1}^K \Lambda_{i}^{0c}(s|r_i,\widehat{\theta})
#'   \right)
#'   f(\mathcal{Y}_i(s)|r_i,\widehat{\theta})
#'   f(r_i|\widehat{\theta})
#'   dr_i
...
#' }
#'
#' For illness-death and competing risks models, \eqn{K = 2}, whereas for a model with a single
#' event, \eqn{K = 1}.
#' For illness-death and single-event models, only the prediction from state (0) to state (1) can be
#' computed, while for competing risks, transitions (0→1) and (0→2) can both be predicted.
#'
#' As in the estimation procedure, the integral over the random effects is computed by QMC
#' approximation and the integral over time by the Gauss-Kronrod quadrature.
#'
#' The 95\% confidence interval of predictions is obtained by the following Monte Carlo algorithm,
#' which can be computationally intensive. For \eqn{L} large enough and \eqn{l = 1,...,L}
#' (e.g. \eqn{L = 1000}):
#' \itemize{
#'   \item Generate
#'   \eqn{\widetilde{\theta}^{(l)} \sim
#'   \mathcal{N}(\widehat{\theta}, V(\widehat{\theta}))},
#'   where \eqn{V(\widehat{\theta})} is the inverse of the Hessian
#'   matrix at \eqn{\widehat{\theta}};
#'
#'   \item Compute
#'   \eqn{\widetilde{\pi}^{(l)}_i(s,t;\widetilde{\theta}^{(l)})}
#'   from the equation above;
#'
#'   \item Compute the 95\% confidence interval from the 2.5th and
#'   97.5th percentiles of the \eqn{L}-sample of
#'   \eqn{\widetilde{\pi}^{(l)}_i(s,t;\widetilde{\theta}^{(l)})}.
#' }
#'
...
#' @param horizon The horizon time of prediction. The function computes the probability of
#' experiencing the event between \code{s} and \code{horizon}. This can be a vector or a single value.
#' @param event An integer indicating for which event the prediction is computed. In the case of a *
#' competing risks model, it can be 1 or 2, whereas for other models it can only be 1.
#' @param CI An integer between 0 and 100 indicating the confidence level (default: \code{95} for a
#' 95% CI). If \code{CI = NULL}, no confidence interval is computed.
#' @param nb.draws An integer giving the number of Monte Carlo draws used to compute the confidence
#' interval (default: \code{1000}).
#'
...
#'#                                   c("ID", "SBPvisit", "age.visit65", "dem","death", "sex",
