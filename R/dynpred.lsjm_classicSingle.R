...
dynpred.lsjm_classicSingle <- function(object,newdata,  s, horizon, event = NULL, IC = 95, nb.draws = 1000){
  ...
  for(i in unique(id)){
    ...
    pred.ponct <- c()
    for(t in window){
      pred.ponct <- c(pred.ponct,predyn_ponct_lsjm_classicSingle(Objectlsjm,  data.long.until.time.s, s, t))
      if(!is.null(IC)){
        pred.boot <- cbind(pred.boot, predyn_boot_lsjm_classicSingle(Objectlsjm, data.long.until.time.s, s, t, nb.draws) )
      }
    }
    if(!is.null(IC)){
