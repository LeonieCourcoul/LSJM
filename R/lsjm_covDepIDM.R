...
#' @import snow
lsjm_covDepIDM <- function(Objectlsmm, Time, deltas, hazard_baseline_01, hazard_baseline_02, hazard_baseline_12, nb.knots.splines,
  ...
  if(is.null(binit) &&  ((nbCase1 + nbCase3)/(nbCase1 + nbCase1bis + nbCase2 + nbCase3) > 0.10)){#nombre de cas 1 et 3 supérieur à 10% => seuil à discuter
    ...
    O_L = as.matrix(0); W_L = as.matrix(0); O_GK_L = as.matrix(0); W_GK_L = as.matrix(0);
    ...
    if(length(id.Case1bis_noCI)>0){
      ...
      Case1bis <- list( "delta2" = data.id.Case1bis$delta2, "Z_01" = Z_01, "Z_02" = Z_02, "Z_12" = Z_12, "Time_T" = data.id.Case1bis$Time_T,
                        ...
                        "y.new" = y.new, "offset" = offset, "B_T_01" = t(B_T_01), "B_T_02" = t(B_T_02), "B_T_12" = t(B_T_12),
                        ...
                        "Bs_T0_01" = Bs_T0_01, "Bs_T0_02" = Bs_T0_02,  "O_base" = O_base, "W_base" = W_base, "O_T" = O_T, "W_T" = t(W_T), "O_GK_T" = O_GK_T,
                        "W_GK_T" = W_GK_T, "O_GK_T0" = O_GK_T0, "W_GK_T0" = W_GK_T0, "O_L" = O_L, "W_L" = t(W_L), "O_GK_L" = O_GK_L,
                        "W_GK_L" = W_GK_L
      )
    ...
    if(length(id.Case2_noCI)>0){
      ...
      list.var <- data.manag.sigma(formGroup,formFixedVar, formRandomVar,data.long.Case2)
      O_base <- list.var$X
      O_base <- as.matrix(O_base)
      W_base <- list.var$U
      W_base <- as.matrix(W_base)
      ...
      if(("variability" %in% sharedtype_01) || ("variability" %in% sharedtype_02) ){
        list.data_T <- data.time(data.id.Case2, data.id.Case2$Time_T, formFixedVar, formRandomVar,timeVar)
        list.data.GK_T <- data.time(list.GK_T$data.id2, c(t(st_T)),formFixedVar, formRandomVar,timeVar)
        O_T <- list.data_T$Xtime; W_T <- list.data_T$Utime
        O_GK_T <- list.data.GK_T$Xtime; W_GK_T <- list.data.GK_T$Utime
    ...
    estimation.noCI <- marqLevAlg(binit_noCI, fn = logR_llh_lsjm_covDepIDM, minimize = FALSE,
                                  ...
                                  Case1 = NULL, Case1bis = Case1bis, Case2 = Case2, Case3 = NULL,
  ...
  O_T = as.matrix(0); W_T = as.matrix(0); O_GK_T = as.matrix(0); W_GK_T = as.matrix(0); O_GK_L_R = as.matrix(0); W_GK_L_R = as.matrix(0);
  O_GK_T0 = as.matrix(0); W_GK_T0 = as.matrix(0);
  if(length(id.Case1)>0){
    ...
    list.var <- data.manag.sigma(formGroup,formFixedVar, formRandomVar,data.long.Case1)
    O_base <- list.var$X
    O_base <- as.matrix(O_base)
    W_base <- list.var$U
    W_base <- as.matrix(W_base)
    ...
    if(("variability" %in% sharedtype_01) || ("variability" %in% sharedtype_02) || ("variability" %in% sharedtype_12)){
      list.data_T <- data.time(data.id.Case1, data.id.Case1$Time_T, formFixedVar, formRandomVar,timeVar)
      list.data.GK_T <- data.time(list.GK_T$data.id2, c(t(st_T)),formFixedVar, formRandomVar,timeVar)
      list.data.GK_L_R <- data.time(list.GK_L_R$data.id2, c(t(st_L_R)),formFixedVar, formRandomVar,timeVar)
      O_T <- list.data_T$Xtime; W_T <- list.data_T$Utime
      O_GK_T <- list.data.GK_T$Xtime; W_GK_T <- list.data.GK_T$Utime
    ...
    for(id.integrale in 1:nbCase1){
      ...
      for(st.integrale in st_L_R_i){
        ...
        if(("variability" %in% sharedtype_01) || ("variability" %in% sharedtype_02) || ("variability" %in% sharedtype_12)){
          list.data.GK_0_stLR <- data.time(list.GK_0_stLR$data.id2, c(t(st_0_stLR_i)),formFixedVar, formRandomVar,timeVar)
          O_0_stLR_i <- list.data.GK_0_stLR$Xtime; W_0_stLR_i <- list.data.GK_0_stLR$Utime
          O_0_LR <- rbind(O_0_LR,O_0_stLR_i); W_0_LR <- rbind(W_0_LR,W_0_stLR_i)
        }
        if(hazard_baseline_01 == "Splines"){
    ...
    Case1 <- list( "delta2" = data.id.Case1$delta2, "Z_01" = Z_01, "Z_02" = Z_02, "Z_12" = Z_12, "Time_T" = data.id.Case1$Time_T,
                   ...
                   "O_T" = O_T, "W_T" = t(W_T), "O_GK_T" = O_GK_T, "W_GK_T" = W_GK_T, "O_GK_L_R" = O_GK_L_R, "W_GK_L_R" = W_GK_L_R,
                   "O_GK_T0" = O_GK_T0, "W_GK_T0" = W_GK_T0,"O_0_LR" = O_0_LR, "W_0_LR" = W_0_LR, "O_base" = O_base, "W_base" = W_base
  ...
  if(length(id.Case1bis)>0){
    ...
    if(("variability" %in% sharedtype_01) || ("variability" %in% sharedtype_02) || ("variability" %in% sharedtype_12)){
      list.data_T <- data.time(data.id.Case1bis, data.id.Case1bis$Time_T, formFixedVar, formRandomVar,timeVar)
      list.data_L <- data.time(data.id.Case1bis, data.id.Case1bis$Time_L, formFixedVar, formRandomVar,timeVar)
      list.data.GK_T <- data.time(list.GK_T$data.id2, c(t(st_T)),formFixedVar, formRandomVar,timeVar)
      list.data.GK_L <- data.time(list.GK_L$data.id2, c(t(st_L)),formFixedVar, formRandomVar,timeVar)
      O_T <- list.data_T$Xtime; W_T <- list.data_T$Utime
      O_L <- list.data_L$Xtime; W_L <- list.data_L$Utime
    ...
    Case1bis <- list( "delta2" = data.id.Case1bis$delta2, "Z_01" = Z_01, "Z_02" = Z_02, "Z_12" = Z_12, "Time_T" = data.id.Case1bis$Time_T,
                      ...
                      "X_L" = X_L, "U_L" = t(U_L), "Xslope_L" = Xslope_L, "Uslope_L" = t(Uslope_L), "Time_T0" = Time_T0,
                      "st_T0" = t(st_T0), "X_GK_T0" = X_GK_T0, "U_GK_T0" = U_GK_T0, "Xslope_GK_T0" = Xslope_GK_T0, "Uslope_GK_T0" = Uslope_GK_T0,
                      "Bs_T0_01" = Bs_T0_01, "Bs_T0_02" = Bs_T0_02, "O_T" = O_T, "W_T" = t(W_T), "O_GK_T" = O_GK_T, "W_GK_T" = W_GK_T,
                      "O_L" = O_L, "W_L" = t(W_L), "O_GK_L" = O_GK_L, "W_GK_L" = W_GK_L, "O_GK_T0" = O_GK_T0, "W_GK_T0" = W_GK_T0,
  ...
  if(length(id.Case2)>0){
    ...
    if(("variability" %in% sharedtype_01) || ("variability" %in% sharedtype_02) ){
      list.data_T <- data.time(data.id.Case2, data.id.Case2$Time_T, formFixedVar, formRandomVar,timeVar)
      list.data.GK_T <- data.time(list.GK_T$data.id2, c(t(st_T)),formFixedVar, formRandomVar,timeVar)
      O_T <- list.data_T$Xtime; O_T <- list.data_T$Utime
      O_GK_T <- list.data.GK_T$Xtime; W_GK_T <- list.data.GK_T$Utime
    ...
    Case2 <- list( "delta2" = data.id.Case2$delta2, "Z_01" = Z_01, "Z_02" = Z_02, "Z_12" = Z_12, "Time_T" = data.id.Case2$Time_T,
                   ...
                   "O_GK_T" = O_GK_T, "W_GK_T" = W_GK_T, "O_T" = O_T, "W_T" = t(W_T), "O_GK_T0" = O_GK_T0, "W_GK_T0" = W_GK_T0,
                   "O_base" = O_base, "W_base" = W_base
    )
  ...
  O_GK_T = as.matrix(0); W_GK_T = as.matrix(0); O_T = as.matrix(0); W_T = as.matrix(0);
  O_GK_L_T = as.matrix(0); W_GK_L_T = as.matrix(0); O_0_LT = as.matrix(0); W_0_LT = as.matrix(0);
  if(length(id.Case3)>0){
    ...
    list.var <- data.manag.sigma(formGroup,formFixedVar, formRandomVar,data.long.Case3)
    O_base <- list.var$X
    O_base <- as.matrix(O_base)
    W_base <- list.var$U
    W_base <- as.matrix(W_base)
    ...
    if(("value" %in% sharedtype_01) || ("value" %in% sharedtype_02) || ("value" %in% sharedtype_12)){
      list.data_T <- data.time(data.id.Case3, data.id.Case3$Time_T, formFixedVar, formRandomVar,timeVar)
      list.data.GK_T <- data.time(list.GK_T$data.id2, c(t(st_T)),formFixedVar, formRandomVar,timeVar)
      list.data.GK_L_T <- data.time(list.GK_L_T$data.id2, c(t(st_L_T)),formFixedVar, formRandomVar,timeVar)
      O_T <- list.data_T$Xtime; W_T <- list.data_T$Utime
      O_GK_T <- list.data.GK_T$Xtime; W_GK_T <- list.data.GK_T$Utime
    ...
    for(id.integrale in 1:nbCase3){
      ...
      for(st.integrale in st_L_T_i){
        ...
        if(("variability" %in% sharedtype_01) || ("variability" %in% sharedtype_02) || ("variability" %in% sharedtype_12)){
          list.data.GK_0_stLT <- data.time(list.GK_0_stLT$data.id2, c(t(st_0_stLT_i)),formFixedVar, formRandomVar,timeVar)
    ...
    Case3 <- list( "delta2" = data.id.Case3$delta2, "Z_01" = Z_01, "Z_02" = Z_02, "Z_12" = Z_12, "Time_T" = data.id.Case3$Time_T,
                   ...
                   "O_T" = O_T, "W_T" = t(W_T), "O_GK_T" = O_GK_T, "W_GK_T" = W_GK_T, "O_GK_L_T" = O_GK_L_T, "W_GK_L_T" = W_GK_L_T,
                   "O_GK_T0" = O_GK_T0, "W_GK_T0" = W_GK_T0, "O_0_LT" = O_0_LT, "W_0_LT" = W_0_LT, "O_base" = O_base, "W_base" = W_base
    )
  ...
  estimation1 <- marqLevAlg(binit, fn = logR_llh_lsjm_covDepIDM, minimize = FALSE,
                            ...
                            ord.splines = nb.knots.splines + 2, nb.beta = nb.beta, Zq = Zq, nb_pointsGK = nb_pointsGK,
                            nb.e.a = nb.e.a, S = S1, wk = gaussKronrod()$wk, rep_wk = rep(gaussKronrod()$wk, length(gaussKronrod()$wk)), sk_GK = gaussKronrod()$sk, nb.alpha = nb.alpha,
                            Case1 = Case1, Case1bis = Case1bis, Case2 = Case2, Case3 = Case3,
