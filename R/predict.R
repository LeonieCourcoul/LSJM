...
#'
#' This function computes different predicted quantities. For a \code{lsmm} object, it is possible
#' to compute, for each subject, their random effects, their value of the marker and the residual
#' variability (for each measurement time)).
#' For a \code{lsjm} object we could also compute cumulative hazard function for each risk
#' transition.
#'
#'
#' @param object Either a \code{lsmm} object or a \code{lsjm} object
#' @param which A vector of characters to indicate which predictions are computed.
#' \code{"RE"} corresponds to the random effects, \code{"Y"} to the marker and \code{"Cum"} to the
#' cumulative risk function(s) (only in the case of a \code{lsjm} object)
#' @param data.long A dataframe containing the longitudinal data for making predictions.
#' @param Objectranef Optional \code{ranef} object containing the predicted random effects for each
#' individual. The Default is NULL and the predict functions should compute the random effects.
...
#' \item{\code{predictY}}{A table with predicted marker and residual variability for each subjects
#' and at each measurement time.}
#' \item{\code{predictCum_01}}{A table for predicted cumulative hazard function for transition 0-1.}
#' \item{\code{predictCum_02}}{A table for predicted cumulative hazard function for transition 0-2.}
#' \item{\code{predictCum_12}}{A table for predicted cumulative hazard function for transition 1-2.}
#' \item{\code{grid.time.Cum}}{A vector of times for which the cumulative hazard functions are
#' computed.}
#' }
#'
#' @examples
