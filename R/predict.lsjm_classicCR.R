...
#' @export
predict.lsjm_classicCR <- function(object, which = "RE", Objectranef = NULL, data.long = NULL){

  Objectlsjm <- object
  if(missing(Objectlsjm)) stop("The argument Objectlsjm must be specified")
  if(!inherits((Objectlsjm),"lsjm_classicCR")) stop("use only \"lsjm_classicCR\" objects")
    ...
    #  print(id.boucle)
                         ...
                         if("Cum" %in% which){
                           ...
                           for(j in 1:nrow(st_calc.sort.unique)){
                             ...
                             if("random effects" %in% x$control$sharedtype_02){
                               pred_haz_02 <- pred_haz_02 + alpha_b_02*random.effects_i$b[1:(x$control$Objectlsmm$control$nb.e.a)]
                             }
  ...
  class(resultat) <- c("predict.lsjm_classicCR")
  resultat
