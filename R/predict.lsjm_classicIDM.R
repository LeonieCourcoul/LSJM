...
predict.lsjm_classicIDM <- function(object, which = "RE", Objectranef = NULL, data.long = NULL, ...){

  Objectlsjm <- object
  if(missing(Objectlsjm)) stop("The argument Objectlsjm must be specified")
  if(!inherits((Objectlsjm),"lsjm_classicIDM")) stop("use only \"lsjm_classicIDM\" objects")
      ...
      results <- foreach(id.boucleCase1 = 1:length(unique(data.long.Case1$id)),
                         ...
                         .packages = c("mvtnorm", "marqLevAlg")) %dopar% {
                           ...
                           if ('Y' %in% which) {
                             ...
                             cv_pred <- cbind(rep(data.id.Case1$id[id.boucleCase1], length(CV)), time.measures_i, CV, rep(sigma_epsilon,length(CV)))
                           }
                           else{
                             cv_pred <- NULL
                           }
      ...
      results <- foreach(id.boucleCase1bis = 1:length(unique(data.long.Case1bis$id)),
                         ...
                         .packages = c("mvtnorm", "marqLevAlg")) %dopar% {
                           ...
                           random.effects_i <- marqLevAlg(binit, fn = re_lsjm_classicIDMCase1Bis, minimize = FALSE,nb.e.a = x$control$Objectlsmm$control$nb.e.a,
                                                          ...
                                                          X_T_i=X_T_i,  U_T_i=U_T_i, Xslope_T_i = Xslope_T_i,  Uslope_T_i = Uslope_T_i,
                                                          X_GK_T_i=X_GK_T_i,  U_GK_T_i=U_GK_T_i,  Xslope_GK_T_i=Xslope_GK_T_i, Uslope_GK_T_i=Uslope_GK_T_i,
                                                          X_L_i=X_L_i,  U_L_i=U_L_i, Xslope_L_i=Xslope_L_i,  Uslope_L_i=Uslope_L_i,
                                                          X_GK_L_i=X_GK_L_i,  U_GK_L_i=U_GK_L_i,  Xslope_GK_L_i=Xslope_GK_L_i,Uslope_GK_L_i=Uslope_GK_L_i,
                           ...
                           if ('Y' %in% which) {
                             ...
                             cv_pred <- cbind(rep(data.id.Case1bis$id[id.boucleCase1bis], length(CV)), time.measures_i, CV,  rep(sigma_epsilon,length(CV)))
                           }
                           else{
                             cv_pred <- NULL
                           }
      ...
      results <- foreach(id.boucleCase2 = 1:length(unique(data.long.Case2$id)),
                         ...
                         .packages = c("mvtnorm", "marqLevAlg")) %dopar% {
                           ...
                           if ('Y' %in% which) {
                             ...
                             cv_pred <- cbind(rep(data.id.Case2$id[id.boucleCase2], length(CV)), time.measures_i, CV,  rep(sigma_epsilon,length(CV)))
                           }
                           else{
                             cv_pred <- NULL
                           }
      ...
      results <- foreach(id.boucleCase3 = 1:length(unique(data.long.Case3$id)),
                         ...
                         .packages = c("mvtnorm", "marqLevAlg")) %dopar% {
                           ...
                           if ('Y' %in% which) {
                             ...
                             cv_pred <- cbind(rep(data.id.Case3$id[id.boucleCase3], length(CV)), time.measures_i, CV,  rep(sigma_epsilon,length(CV)))
                           }
                           else{
                             cv_pred <- NULL
                           }
  ...
  class(resultat) <- c("predict.lsjm_classicIDM")
  resultat
