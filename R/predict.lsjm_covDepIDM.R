...
predict.lsjm_covDepIDM <- function(object, which = "RE", Objectranef = NULL, data.long = NULL, ...){

  Objectlsjm <- object
  if(missing(Objectlsjm)) stop("The argument Objectlsjm must be specified")
  if(!inherits((Objectlsjm),"lsjm_covDepIDM")) stop("use only \"lsjm_covDepIDM\" objects")
    ...
    for(id.integrale in 1:nbCase1){
      ...
      for(st.integrale in st_L_R_i){
        ...
        if(("variability" %in% x$control$sharedtype_01) || ("variability" %in% x$control$sharedtype_02) || ("variability" %in% x$control$sharedtype_12)){
          ...
          O_GK_0_LR <- rbind(O_GK_0_LR,O_0_stLR_i); W_GK_0_LR <- rbind(W_GK_0_LR,W_0_stLR_i)
        }
        if(x$control$hazard_baseline_01 == "Splines"){
    ...
    for(id.integrale in 1:nbCase3){
      ...
      for(st.integrale in st_L_T_i){
        ...
        if(("variability" %in% x$control$sharedtype_01) || ("variability" %in% x$control$sharedtype_02) || ("variability" %in% x$control$sharedtype_12)){
          ...
          O_0_LT <- rbind(O_0_LT,O_0_stLT_i); W_0_LT <- rbind(W_0_LT,W_0_stLT_i)
        }
        if(x$control$hazard_baseline_01 == "Splines"){
    ...
    for(id_boucle in 1:nrow(data.id)){
      ...
      for(j in 1:length((grid.time))){
        ...
        if("variability" %in% x$control$sharedtype_01 || "variability" %in% x$control$sharedtype_02 || "variability" %in% x$control$sharedtype_12){
          ...
          Sigma.current.GK <- exp(omega%*%t(Os.j) + as.matrix(random.effects.Predictions[which(random.effects.Predictions$id == data.id$id[id_boucle]),(x$control$Objectlsmm$control$nb.e.a+2):(x$control$Objectlsmm$control$nb.e.a+x$control$Objectlsmm$control$nb.e.a.sigma+1)])%*%t(Ws.j))
          if("variability" %in% x$control$sharedtype_01){
