...
predict.lsjm_interintraIDM <- function(object, which = "RE", Objectranef = NULL, data.long = NULL, ...){

  Objectlsjm <- object
  if(missing(Objectlsjm)) stop("The argument Objectlsjm must be specified")
  if(!inherits((Objectlsjm),"lsjm_interintraIDM")) stop("use only \"lsjm_interintraCR\" objects")
  ...
  alpha.inter_01 <- 0 ; alpha.inter_02 <- 0; alpha.inter_12 <- 0;  alpha.intra_01 <- 0; alpha.intra_02 <- 0; alpha.intra_12 <- 0;
  ...
  if(length(id.Case1)>0){
    ...
    Z_12 <- list.surv$Z
    if(x$control$hazard_baseline_12 == "Gompertz"){Z_12 <- as.matrix(Z_12[,-1,  drop = FALSE])}
    if(x$control$hazard_baseline_01 == "Splines"){
      Z_01 <- as.matrix(Z_01[,-1,  drop = FALSE])
      B_T_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, data.id.Case1$Time_T, ord = 4L)
      Bs_T_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_T)), ord = 4L)
      Bs_L_R_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_L_R)), ord = 4L)
      if(x$control$left_trunc){
    ...
    if(x$control$hazard_baseline_02 == "Splines"){
      ...
      B_T_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, data.id.Case1$Time_T, ord = 4L)
      Bs_T_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_T)), ord = 4L)
      Bs_L_R_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_L_R)), ord = 4L)
      if(x$control$left_trunc){
    ...
    if(x$control$hazard_baseline_12 == "Splines"){
      ...
      B_T_12 <- splineDesign(x$control$knots.hazard_baseline.splines_12, data.id.Case1$Time_T, ord = 4L)
    ...
    for(id.integrale in 1:nbCase1){
      ...
      for(st.integrale in st_L_R_i){
        ...
        if(x$control$hazard_baseline_01 == "Splines"){
          Bs_0_LR_01 <- rbind(Bs_0_LR_01,splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_0_stLR_i)), ord = 4L))
        }
        if(x$control$hazard_baseline_02 == "Splines"){
          Bs_0_LR_02 <- rbind(Bs_0_LR_02,splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_0_stLR_i)), ord = 4L))
        }
        if(x$control$hazard_baseline_12 == "Splines"){
          Bs_0_LR_12 <- rbind(Bs_0_LR_12,splineDesign(x$control$knots.hazard_baseline.splines_12, c(t(st_0_stLR_i)), ord = 4L))
        }
      }
    ...
    if(is.null(Objectranef) || ('RE' %in% which)){
      ...
      results <- foreach(id.boucleCase1 = 1:length(unique(data.long.Case1$id)),
                         ...
                         .packages = c("mvtnorm", "marqLevAlg")) %dopar% {
                           ...
                           ID.visit_i <- ID.visit[offset[id.boucleCase1]:(offset[id.boucleCase1+1]-1)]
                           offset_ID_i <- as.vector(c(1, 1 + cumsum(tapply(ID.visit_i, ID.visit_i, length))))
                           ...
                           random.effects_i <- marqLevAlg(binit, fn = re_lsjm_interintraIDMCase1, minimize = FALSE, nb.e.a = x$control$Objectlsmm$control$nb.e.a,
                                                          variability_inter_visit = x$control$Objectlsmm$control$var_inter,
                                                          ...
                                                          alpha_y_slope = alpha_y_slope, alpha_inter_intra = alpha_inter_intra, alpha_b_01 = alpha_b_01, alpha_b_02 = alpha_b_02,alpha_b_12 = alpha_b_12,
                                                          alpha_z = alpha_z,  gamma_z0 = gamma_z0,  beta = beta,  beta_slope = beta_slope,
                                                          wk = wk, rep_wk = rep_wk, mu.inter = mu.inter , sigma.epsilon.inter = sigma.epsilon.inter, mu.intra = mu.intra,sigma.epsilon.intra = sigma.epsilon.intra,
                                                          ...
                                                          X_GK_L_R_i = X_GK_L_R_i,  U_GK_L_R_i = U_GK_L_R_i,  Xslope_GK_L_R_i = Xslope_GK_L_R_i,  Uslope_GK_L_R_i = Uslope_GK_L_R_i,
                                                          X_GK_0_LR_i = X_GK_0_LR_i,  U_GK_0_LR_i = U_GK_0_LR_i,  Xslope_GK_0_LR_i = Xslope_GK_0_LR_i,  Uslope_GK_0_LR_i = Uslope_GK_0_LR_i,
                                                          ...
                                                          X_base_i=X_base_i,  U_base_i=U_base_i,   y_i=y_i, offset_ID_i = offset_ID_i,
                           ...
                           if ('Y' %in% which) {
                             ...
                             time.measures_i <- unique(time.measures[offset[id.boucleCase1]:(offset[id.boucleCase1+1]-1)])
                             if(x$control$Objectlsmm$control$var_inter && x$control$Objectlsmm$control$var_intra){
  ...
  if(length(id.Case1bis)>0){
    ...
    ID.visit <- data.long.Case1bis[all.vars(x$control$Objectlsmm$control$formGroupVisit)][,1];
    offset_ID <- c()
    len_visit <- c(0)
    Ind <- nrow(data.id.Case1bis)
    for(oo in 1:Ind){
      ID.visit_i <- ID.visit[offset[oo]:(offset[oo+1]-1)]
      offset_ID_i <- as.vector(c(1, 1 + cumsum(tapply(ID.visit_i, ID.visit_i, length))))
      len_visit <- c(len_visit,length(unique(ID.visit_i)))
    ...
    if(x$control$hazard_baseline_01 == "Splines"){
      Z_01 <- as.matrix(Z_01[,-1,  drop = FALSE])
      B_T_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, data.id.Case1bis$Time_T, ord = 4L)
      B_L_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, data.id.Case1bis$Time_L, ord = 4L)
      Bs_T_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_T)), ord = 4L)
      Bs_L_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_L)), ord = 4L)
      if(x$control$left_trunc){
        Bs_T0_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_T0)), ord = 4L)
      }
    }
    if(x$control$hazard_baseline_02 == "Splines"){
      Z_02 <- as.matrix(Z_02[,-1,  drop = FALSE])
      B_T_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, data.id.Case1bis$Time_T, ord = 4L)
      B_L_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, data.id.Case1bis$Time_L, ord = 4L)
      Bs_T_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_T)), ord = 4L)
      Bs_L_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_L)), ord = 4L)
      if(x$control$left_trunc){
        Bs_T0_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_T0)), ord = 4L)
      }
    }
    if(x$control$hazard_baseline_12 == "Splines"){
      Z_12 <- as.matrix(Z_12[,-1,  drop = FALSE])
      B_T_12 <- splineDesign(x$control$knots.hazard_baseline.splines_12, data.id.Case1bis$Time_T, ord = 4L)
      B_L_12 <- splineDesign(x$control$knots.hazard_baseline.splines_12, data.id.Case1bis$Time_L, ord = 4L)
      Bs_T_12 <- splineDesign(x$control$knots.hazard_baseline.splines_12, c(t(st_T)), ord = 4L)
      Bs_L_12 <- splineDesign(x$control$knots.hazard_baseline.splines_12, c(t(st_L)), ord = 4L)
    }
    ...
    if(is.null(Objectranef) || ('RE' %in% which)){
      ...
      results <- foreach(id.boucleCase1bis = 1:length(unique(data.long.Case1bis$id)),
                         ...
                         .packages = c("mvtnorm", "marqLevAlg")) %dopar% {
                           ...
                           ID.visit_i <- ID.visit[offset[id.boucleCase1bis]:(offset[id.boucleCase1bis+1]-1)]
                           offset_ID_i <- as.vector(c(1, 1 + cumsum(tapply(ID.visit_i, ID.visit_i, length))))
                           len_visit_i <- length(unique(ID.visit_i))
                           ...
                           random.effects_i <- marqLevAlg(binit, fn = re_lsjm_interintraIDMCase1Bis, minimize = FALSE,nb.e.a = x$control$Objectlsmm$control$nb.e.a,
                                                          variability_inter_visit = x$control$Objectlsmm$control$var_inter,
                                                          variability_intra_visit = x$control$Objectlsmm$control$var_intra,
                                                          Sigma.re = MatCov,
                                                          sharedtype = sharedtype, HB = HB, W_G = W_G, nb_pointsGK = x$control$nb_pointsGK,
                          ...
                          if ('Y' %in% which) {
                            CV <- X_base_i %*% beta + U_base_i %*% random.effects_i$b[1:(x$control$Objectlsmm$control$nb.e.a)]
                            time.measures_i <- unique(time.measures[offset[id.boucleCase1bis]:(offset[id.boucleCase1bis+1]-1)])
                            if(x$control$Objectlsmm$control$var_inter && x$control$Objectlsmm$control$var_intra){
                              Varia.inter <- exp(mu.inter + random.effects_i$b[x$control$Objectlsmm$control$nb.e.a+1])
                              Varia.intra <- exp(mu.intra + random.effects_i$b[x$control$Objectlsmm$control$nb.e.a+2])
                            }
  ...
  if(length(id.Case2)>0){
    ...
    ID.visit <- data.long.Case2[all.vars(x$control$Objectlsmm$control$formGroupVisit)][,1];
    offset_ID <- c()
    len_visit <- c(0)
    Ind <- nrow(data.id.Case2)
    for(oo in 1:Ind){
      ID.visit_i <- ID.visit[offset[oo]:(offset[oo+1]-1)]
      offset_ID_i <- as.vector(c(1, 1 + cumsum(tapply(ID.visit_i, ID.visit_i, length))))
      len_visit <- c(len_visit,length(unique(ID.visit_i)))
    ...
    if(x$control$hazard_baseline_01 == "Splines"){
      Z_01 <- as.matrix(Z_01[,-1,  drop = FALSE])
      B_T_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, data.id.Case2$Time_T, ord = 4L)
      Bs_T_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_T)), ord = 4L)
      if(x$control$left_trunc){
        Bs_T0_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_T0)), ord = 4L)
      }
    ...
    if(x$control$hazard_baseline_02 == "Splines"){
      Z_02 <- as.matrix(Z_02[,-1,  drop = FALSE])
      B_T_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, data.id.Case2$Time_T, ord = 4L)
      Bs_T_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_T)), ord = 4L)
      if(x$control$left_trunc){
        Bs_T0_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_T0)), ord = 4L)
      }
    ...
    if(is.null(Objectranef) || ('RE' %in% which)){
      ...
      results <- foreach(id.boucleCase2 = 1:length(unique(data.long.Case2$id)),
                         ...
                         .packages = c("mvtnorm", "marqLevAlg")) %dopar% {
                           ...
                           ID.visit_i <- ID.visit[offset[id.boucleCase2]:(offset[id.boucleCase2+1]-1)]
                           offset_ID_i <- as.vector(c(1, 1 + cumsum(tapply(ID.visit_i, ID.visit_i, length))))
                           len_visit_i <- length(unique(ID.visit_i))
                           ...
                           random.effects_i <- marqLevAlg(binit, fn = re_lsjm_interintraIDMCase2, minimize = FALSE,
                                                          ...
                                                          nb.e.a = x$control$Objectlsmm$control$nb.e.a,
                                                          variability_inter_visit = x$control$Objectlsmm$control$var_inter,
                                                          ...
                                                          alpha_z = alpha_z,  gamma_z0 = gamma_z0,  beta = beta,  beta_slope = beta_slope,
                                                          wk = wk, mu.inter = mu.inter , sigma.epsilon.inter = sigma.epsilon.inter, mu.intra = mu.intra,sigma.epsilon.intra = sigma.epsilon.intra,
                           ...
                           if ('Y' %in% which) {
                             CV <- X_base_i %*% beta + U_base_i %*% random.effects_i$b[1:(x$control$Objectlsmm$control$nb.e.a)]
                             time.measures_i <- unique(time.measures[offset[id.boucleCase2]:(offset[id.boucleCase2+1]-1)])
                             if(x$control$Objectlsmm$control$var_inter && x$control$Objectlsmm$control$var_intra){
                               Varia.inter <- exp(mu.inter + random.effects_i$b[x$control$Objectlsmm$control$nb.e.a+1])
                               Varia.intra <- exp(mu.intra + random.effects_i$b[x$control$Objectlsmm$control$nb.e.a+2])
                             }
  ...
  Xslope_GK_L_T = as.matrix(0); Uslope_GK_L_T = as.matrix(0);
  Time_L_T = c(0);
  ...
  if(length(id.Case3)>0){
    ...
    ID.visit <- data.long.Case3[all.vars(x$control$Objectlsmm$control$formGroupVisit)][,1];
    offset_ID <- c()
    len_visit <- c(0)
    Ind <- nrow(data.id.Case3)
    for(oo in 1:Ind){
      ID.visit_i <- ID.visit[offset[oo]:(offset[oo+1]-1)]
      offset_ID_i <- as.vector(c(1, 1 + cumsum(tapply(ID.visit_i, ID.visit_i, length))))
      len_visit <- c(len_visit,length(unique(ID.visit_i)))
    ...
    if(x$control$hazard_baseline_01 == "Splines"){
      Z_01 <- as.matrix(Z_01[,-1,  drop = FALSE])
      B_T_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, data.id.Case3$Time_T, ord = 4L)
      Bs_T_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_T)), ord = 4L)
      B_GK_L_T_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_L_T)), ord = 4L)
      if(x$control$left_trunc){
        Bs_T0_01 <- splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_T0)), ord = 4L)
      }
    }
    if(x$control$hazard_baseline_02 == "Splines"){
      Z_02 <- as.matrix(Z_02[,-1,  drop = FALSE])
      B_T_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, data.id.Case3$Time_T, ord = 4L)
      Bs_T_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_T)), ord = 4L)
      B_GK_L_T_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_L_T)), ord = 4L)
      if(x$control$left_trunc){
        Bs_T0_02 <- splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_T0)), ord = 4L)
      }
    }
    if(x$control$hazard_baseline_12 == "Splines"){
      Z_12 <- as.matrix(Z_12[,-1,  drop = FALSE])
      B_T_12 <- splineDesign(x$control$knots.hazard_baseline.splines_12, data.id.Case3$Time_T, ord = 4L)
      Bs_T_12 <- splineDesign(x$control$knots.hazard_baseline.splines_12, c(t(st_T)), ord = 4L)
      B_GK_L_T_12 <- splineDesign(x$control$knots.hazard_baseline.splines_12, c(t(st_L_T)), ord = 4L)
    }
    ...
    for(id.integrale in 1:nbCase3){
      ...
      for(st.integrale in st_L_T_i){
        ...
        if(x$control$hazard_baseline_01 == "Splines"){
          Bs_0_LT_01 <- rbind(Bs_0_LT_01,splineDesign(x$control$knots.hazard_baseline.splines_01, c(t(st_0_stLT_i)), ord = 4L))
        }
        if(x$control$hazard_baseline_02 == "Splines"){
          Bs_0_LT_02 <- rbind(Bs_0_LT_02,splineDesign(x$control$knots.hazard_baseline.splines_02, c(t(st_0_stLT_i)), ord = 4L))
        }
        if(x$control$hazard_baseline_12 == "Splines"){
          Bs_0_LT_12 <- rbind(Bs_0_LT_12,splineDesign(x$control$knots.hazard_baseline.splines_12, c(t(st_0_stLT_i)), ord = 4L))
        }
      }
    ...
    if(is.null(Objectranef) || ('RE' %in% which)){
      ...
      results <- foreach(id.boucleCase3 = 1:length(unique(data.long.Case3$id)),
                         ...
                         .packages = c("mvtnorm", "marqLevAlg")) %dopar% {
                           ...
                           ID.visit_i <- ID.visit[offset[id.boucleCase3]:(offset[id.boucleCase3+1]-1)]
                           offset_ID_i <- as.vector(c(1, 1 + cumsum(tapply(ID.visit_i, ID.visit_i, length))))
                           len_visit_i <- length(unique(ID.visit_i))
                           ...
                           random.effects_i <- marqLevAlg(binit, fn = re_lsjm_interintraIDMCase3, minimize = FALSE,nb.e.a = x$control$Objectlsmm$control$nb.e.a,
                                                          variability_inter_visit = x$control$Objectlsmm$control$var_inter,
                                                          variability_intra_visit = x$control$Objectlsmm$control$var_intra,
                                                          Sigma.re = MatCov,
                                                          sharedtype = sharedtype, HB = HB, W_G = W_G, nb_pointsGK = x$control$nb_pointsGK,
                                                          ...
                                                          X_T_i=X_T_i,  U_T_i=U_T_i, Xslope_T_i = Xslope_T_i,  Uslope_T_i = Uslope_T_i,
                                                          X_GK_T_i = X_GK_T_i,  U_GK_T_i = U_GK_T_i,  Xslope_GK_T_i = Xslope_GK_T_i,Uslope_GK_T_i = Uslope_GK_T_i,
                                                          X_GK_L_T_i = X_GK_L_T_i,  U_GK_L_T_i = U_GK_L_T_i,  Xslope_GK_L_T_i = Xslope_GK_L_T_i,  Uslope_GK_L_T_i = Uslope_GK_L_T_i,
                                                          X_GK_0_LT_i = X_GK_0_LT_i,  U_GK_0_LT_i = U_GK_0_LT_i,  Xslope_GK_0_LT_i = Xslope_GK_0_LT_i,  Uslope_GK_0_LT_i = Uslope_GK_0_LT_i,
                                                          X_GK_T0_i = X_GK_T0_i,  U_GK_T0_i = U_GK_T0_i,  Xslope_GK_T0_i = Xslope_GK_T0_i,  Uslope_GK_T0_i = Uslope_GK_T0_i,
                           ...
                           if ('Y' %in% which) {
                             CV <- X_base_i %*% beta + U_base_i %*% random.effects_i$b[1:(x$control$Objectlsmm$control$nb.e.a)]
                             time.measures_i <- unique(time.measures[offset[id.boucleCase3]:(offset[id.boucleCase3+1]-1)])
                             if(x$control$Objectlsmm$control$var_inter && x$control$Objectlsmm$control$var_intra){
                               Varia.inter <- exp(mu.inter + random.effects_i$b[x$control$Objectlsmm$control$nb.e.a+1])
                               Varia.intra <- exp(mu.intra + random.effects_i$b[x$control$Objectlsmm$control$nb.e.a+2])
                             }
  ...
  if('Cum' %in% which){
    ...
    for(id_boucle in 1:nrow(data.id)){
      ...
      for(j in 1:length((grid.time))){
        ...
        #browser()
        if(x$control$Objectlsmm$control$var_inter && x$control$Objectlsmm$control$var_intra){
          Varia.inter <- exp(mu.inter + random.effects.Predictions[which(random.effects.Predictions$id ==  data.id$id[id_boucle]),x$control$Objectlsmm$control$nb.e.a+2])
          Varia.intra <- exp(mu.intra + random.effects.Predictions[which(random.effects.Predictions$id ==  data.id$id[id_boucle]),x$control$Objectlsmm$control$nb.e.a+3])
        }
        else{
          if(x$control$Objectlsmm$control$var_inter){
            Varia.inter <- exp(mu.inter + random.effects.Predictions[which(random.effects.Predictions$id ==  data.id$id[id_boucle]),x$control$Objectlsmm$control$nb.e.a+2])
            ...
            if(x$control$Objectlsmm$control$var_intra){
              Varia.intra <- exp(mu.intra + random.effects.Predictions[which(random.effects.Predictions$id ==  data.id$id[id_boucle]),x$control$Objectlsmm$control$nb.e.a+2])
            }
          }
        }
        ...
        if("variability inter" %in% x$control$sharedtype_02){
          pred_haz_02 <- pred_haz_02 + alpha.inter_02*Varia.inter
        }
        if("variability inter" %in% x$control$sharedtype_01){
          pred_haz_01 <- pred_haz_01 + alpha.inter_01*Varia.inter
        }
        if("variability inter" %in% x$control$sharedtype_12){
          pred_haz_12 <- pred_haz_12 + alpha.inter_12*Varia.inter
        }


        if("variability intra" %in% x$control$sharedtype_02){
          pred_haz_02 <- pred_haz_02 + alpha.intra_02*Varia.intra

        }
        if("variability intra" %in% x$control$sharedtype_01){
          pred_haz_01 <- pred_haz_01 + alpha.intra_01*Varia.intra

        }
        if("variability intra" %in% x$control$sharedtype_12){
          pred_haz_12 <- pred_haz_12 + alpha.intra_12*Varia.intra
        ...
        if("value" %in% x$control$sharedtype_01 || "value" %in% x$control$sharedtype_02 || "value" %in% x$control$sharedtype_12){
          ...
          current.GK <- beta%*%t(Xs.j) + as.matrix(random.effects.Predictions[which(random.effects.Predictions$id ==  data.id$id[id_boucle]),2:(x$control$Objectlsmm$control$nb.e.a+1)])%*%t(Us.j)
          if("value" %in% x$control$sharedtype_01){
        ...
        if("slope" %in% x$control$sharedtype_01 || "slope" %in% x$control$sharedtype_02 || "slope" %in% x$control$sharedtype_12){
          ...
          bslope <- as.matrix(random.effects.Predictions[which(random.effects.Predictions$id ==  data.id$id[id_boucle]),2:(x$control$Objectlsmm$control$nb.e.a+1)])
          ...
          if("slope" %in% x$control$sharedtype_12){
            pred_haz_12 <- pred_haz_12 + alpha.slope_12*slope.GK
          }
        }
        ...
        if(x$control$hazard_baseline_12 == "Weibull"){
          st_j <- st_calc.sort.unique[j,]
          h_0.GK_12 <- shape_12*(st_j**(shape_12-1))*wk
        }
        if(x$control$hazard_baseline_12 == "Gompertz"){
          st_j <- st_calc.sort.unique[j,]
        ...
        if(x$control$hazard_baseline_12 == "Splines"){
          st_j <- st_calc.sort.unique[j,]
          Bs_j <- splineDesign(x$control$knots.hazard_baseline.splines_12, st_j, ord = 4L)
          #Bs_j <- Bs[(x$control$nb_pointsGK*(j-1)+1):(x$control$nb_pointsGK*j),]
  ...
  resultat <- list(predictRE = random.effects.Predictions, predictY = cv.Pred, predictCum_01 = Cum_risk_01, predictCum_02 = Cum_risk_02, predictCum_12 = Cum_risk_12, grid.time.Cum = grid.time)
  class(resultat) <- c("predict.lsjm_interintraIDM")
  resultat
