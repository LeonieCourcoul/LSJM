...
predict.lsjm_interintraIDM <- function(Objectlsmm, which = "RE", Objectranef = NULL, data.long = NULL){

  if(missing(Objectlsmm)) stop("The argument Objectlsmm must be specified")
  if(!inherits((Objectlsmm),"lsjm_interintraIDM")) stop("use only \"lsjm_interintraIDM\" objects")
    ...
    for(id_boucle in 1:nrow(data.id)){
      ...
      for(j in 1:length((grid.time))){
        ...
        if(x$control$Objectlsmm$control$var_inter && x$control$Objectlsmm$control$var_intra){
          ...
          Varia.intra <- exp(mu.inter + random.effects.Predictions[which(random.effects.Predictions$id ==  data.id$id[id_boucle]),x$control$Objectlsmm$control$nb.e.a+3])
        }
        else{
          if(x$control$Objectlsmm$control$var_inter){
            Varia.inter <- exp(mu.inter + random.effects.Predictions[which(random.effects.Predictions$id ==  data.id$id[id_boucle]),x$control$Objectlsmm$control$nb.e.a+2])
            ...
            if(x$control$Objectlsmm$control$var_intra){
              Varia.intra <- exp(mu.inter + random.effects.Predictions[which(random.effects.Predictions$id ==  data.id$id[id_boucle]),x$control$Objectlsmm$control$nb.e.a+2])
            }
          }
        }
