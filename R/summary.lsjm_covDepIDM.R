...
summary.lsjm_covDepIDM <- function(object,...)
{
  x <- object
  ...
  cat("Joint location-scale model with covariate and time-dependent residual variability for quantitative outcome and an illness-death model", "\n")
  ...
  if("variability" %in% x$control$sharedtype_12){
    ...
    alpha.var_12.se <- param.se[curseur]
    alpha.var_12.name <- param.names[curseur]
    curseur <- curseur + 1
  }
  ...
  e1_surv_tab <- rbind(e1_share_current_tab, e1_share_slope_tab,e1_share_var_tab,  e1_alpha_tab)
  ...
  e2_surv_tab <- rbind(e2_share_current_tab, e2_share_slope_tab, e2_share_var_tab, e2_alpha_tab)
  ...
  if(c("variability") %in% x$control$sharedtype_12){
    e12_share_var_tab <- matrix(nrow = 1, ncol = 4)
    e12_share_var_tab[,1] <- alpha.var_12
    e12_share_var_tab[,2] <- alpha.var_12.se
    e12_share_var_tab[,3] <- e12_share_var_tab[,1]/e12_share_var_tab[,2]
    e12_share_var_tab[,4] <- 1 - pchisq(e12_share_var_tab[,3]**2,1)
    e12_names_tab <- c(e12_names_tab, alpha.var_12.name)
  }
  if(x$control$hazard_baseline_12 == "Splines"){
  ...
  e12_surv_tab <- rbind(e12_share_current_tab, e12_share_slope_tab, e12_share_var_tab, e12_alpha_tab)
